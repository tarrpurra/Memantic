                    ...
                    <div className="text-center">
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        >
                          ...
                          {meme.votes}
                          {!isAuthenticated && <span className="ml-1 text-xs">(Login to vote)</span>}
                        </Button>
                      </div>
                    </div>
                  </CardContent>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredMemes.map((meme) => (
              <Card key={meme.id} className="group hover:shadow-glow transition-all duration-300">
                <CardContent className="p-6">
                  <div className="text-center">
                    <div className="text-5xl mb-4 group-hover:animate-pulse">{meme.emoji}</div>
                    ...
                    <Button 
                      size="sm" 
                      variant={isAuthenticated ? "outline" : "ghost"}
                      className={`w-full ${!isAuthenticated ? "opacity-50 cursor-not-allowed" : ""}`}
                      onClick={() => handleVote(meme.id)}
                      disabled={!isAuthenticated}
                    >
                      <ArrowUp className="w-4 h-4 mr-2" />
                      {meme.votes} votes
                      {!isAuthenticated && <span className="ml-1 text-xs">(Login)</span>}
                    </Button>
                  </div>
                </CardContent>
              </Card>
